...
resource "aws_security_group" "Red_PrivateSG" {
  vpc_id = aws_vpc.red_vpc.id
...
resource "aws_route_table" "Red_PublicRT" {
  vpc_id = aws_internet_gateway.Red_IGW
  ...
  association {
    subnet_id = aws_subnet.Red_PublicSubnet.id
  }
}

# Private Route Table
resource "aws_route_table" "Red_PrivateRT" {
  vpc_id = aws_vpc.red_vpc.id

  association {
    subnet_id = aws_subnet.Red_PrivateSubnet.id
  }
}
